...
def batch_convert_dicom_to_nifti(
    ...
    convert_by_subfolder_level: int = 0,
    ...
    if convert_by_subfolder_level > 0:
        # get a list of new base paths from all subdirectories of base_path
        base_paths = [x[0] for x in os.walk(base_path)]
        print(f'base_paths = {base_paths}')
        convert_by_subfolder_level -= 1
        for base_path in base_paths:
            _ = batch_convert_dicom_to_nifti(
                base_path,
                output_base_dir,
                structure_map,
                df_kwargs,
    ...
    if len(paired_series) == 0:
        logging.warning(f'No DICOM series found in {base_path}')
        return None
